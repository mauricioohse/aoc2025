...
pair_t last_merged = {0};

void save_last_merge(int p1, int p2)
{
    last_merged.p1_idx=p1;
    last_merged.p2_idx=p2;
}
...
int main()
{
    ...
    for( int i = 0;  i < 1000*1000; i++)
    {
        ...
        else if(p1_ckt != 0 && p2_ckt != 0 && p1_ckt != p2_ckt)
        {
        ...
        // if(i>1000 && i%1000 == 0)
        // {
        //     // checks once in a while if all the circuit are the same
        //     int same = 0;
        //     for (int i = 0; i < 1000; i++)
        //     {
        //         if (same == 0 && p.point_to_circuit_map[i] != 0)
        //             same = p.point_to_circuit_map[i];

        //         if (same != p.point_to_circuit_map[i] && same != 0) continue;
        //     }
        //     // if we reach here, it is becase same was always equal in all points, therefore everything is in once circuit!
        //     break;
        // }
    }
    ...
    int3 p1 = p.points[last_merged.p1_idx];
    int3 p2 = p.points[last_merged.p2_idx];
    printf("Part2 result:\n Last merged Xs: %lld x %lld = %lld", p1.p[0], p2.p[0], p1.p[0]*p2.p[0] ); // 6890193200 too high!
